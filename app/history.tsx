...
import * as MediaLibrary from 'expo-media-library';
import { FileText, Camera as CameraIcon }

from 'react-native-vector-icons/Feather';

const ImageGridScreen = () => {
  const [recentImages, setRecentImages] = useState(Array(6).fill(null));
  ...
  const saveRecentImages = async (images) => {
    try {
      await AsyncStorage.setItem('recentImages', JSON.stringify(images));
    } catch (error) {
  ...
  const addNewImage = (uri) => {
    const updatedImages = [uri, ...recentImages.slice(0, 5)];
    setRecentImages(updatedImages);
    saveRecentImages(updatedImages);
  };

  // Handle document/gallery button press
  ...
  // Handle press on an image
  const handleImagePress = (index) => {
    if (recentImages[index]) {
      Alert.alert(
        'Image Options',
        'What would you like to do?',
        [
          { text: 'View', onPress: () => console.log('View pressed') },
          { text: 'Remove', onPress: () => removeImage(index) },
          { text: 'Cancel', style: 'cancel' }
        ]
      );
    }
