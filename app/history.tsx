...
const HistoryScreen = () => {
  ...
  const toggleStar = (id: string) => {
    ...
    setFilteredActivities(
      filterCategory 
        ? updatedActivities.filter((a) => a.category === filterCategory || (!a.category && filterCategory === "Uncategorized"))
        : updatedActivities
    );
    
    if (selectedActivity && selectedActivity.id === id) {
      setSelectedActivity({ ...selectedActivity, isStarred: !selectedActivity.isStarred });
    }
  ...
  const deleteActivity = (id: string) => {
    const updatedActivities = activityLog.filter((activity) => activity.id !== id);
    setActivityLog(updatedActivities);
    setFilteredActivities(
      filterCategory 
        ? updatedActivities.filter((a) => a.category === filterCategory || (!a.category && filterCategory === "Uncategorized"))
  ...
  const shareActivity = async (activity: Activity) => {
    try {
      ...
      Alert.alert("Sharing failed", "There was an error sharing this activity.");
    }
  };
...
`;
      ...
      Alert.alert("Export failed", "There was an error exporting this activity.");
    }
  };
  ...
  const exportAllActivities = async () => {
    try {
      ...
      activityLog.forEach((activity, index) => {
        ...
        fileContent += `Date: ${new Date(activity.timestamp || 0).toLocaleString()}\n`;
        fileContent += `Category: ${activity.category || "Uncategorized"}\n`;
        fileContent += `Starred: ${activity.isStarred ? "Yes" : "No"}\n\n`;
        fileContent += "--------------------\n\n";
      });
      
      const fileUri = `${FileSystem.cacheDirectory}activity_log_${Date.now()}.txt`;
      await FileSystem.writeAsStringAsync(fileUri, fileContent);
      ...
      Alert.alert("Export failed", "There was an error exporting your activities.");
    }
  };
  ...
  const renderActivityItem = (activity: Activity) => (
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        if (voiceEnabled) {
          let speechText = `Previewing activity: ${activity.action || "No action"}.`;
          if (activity.text) {
    ...
    >
      <View style={styles.historyItemHeader}>
        <View style={styles.activityMeta}>
          <Text style={[styles.historyAction, { color: colors.text }]}>{activity.action}</Text>
          {activity.category && (
          ...
          <>
            {filteredActivities.length === 0 ? (
              <View style={styles.emptyStateContainer}>
                <BookOpen size={60} color={colors.subtext} style={{ marginBottom: 16 }} />
              ...
              >
                {filterCategory && (
                  <View style={[styles.filterBanner, { backgroundColor: colors.primary + '20' }]}>
                ...
                {sortedDates.map(dateStr => (
                  <View key={dateStr}>
                    <View style={styles.dateHeader}>
                      ...
                      <Text style={[styles.dateHeaderText, { color: colors.text }]}>{dateStr}</Text>
                    </View>
          ...
          >
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleContainer}>
                ...
                {selectedActivity?.isStarred && (
                  <Star size={16} color="#FFD700" fill="#FFD700" style={{ marginLeft: 8 }} />
                )}
              </View>
            ...
            <ScrollView style={styles.modalBody}>
              <View style={styles.detailItem}>
                <Text style={[styles.detailLabel, { color: colors.subtext }]}>Action</Text>
            ...
            <View style={[styles.modalFooter, { borderTopColor: colors.border }]}>
              <View style={styles.actionButtons}>
                ...
                >
                  <Star 
                    size={20} 
                    color={selectedActivity?.isStarred ? "#FFD700" : colors.subtext}
                    fill={selectedActivity?.isStarred ? "#FFD700" : "transparent"}
                  />
        ...
        >
          <View style={styles.confirmModalOverlay}>
            <View style={[styles.confirmModalContent, { backgroundColor: colors.card }]}>
              ...
              <Text style={[styles.confirmModalMessage, { color: colors.subtext }]}>
                Are you sure you want to delete this activity? This action cannot be undone.
              </Text>
              <View style={styles.confirmModalActions}>
                <TouchableOpacity
                  style={[styles.confirmModalButton, { borderColor: colors.border }]}
                  onPress={() => setShowDeleteConfirm(false)}
                >
                  <Text style={[styles.confirmModalButtonText, { color: colors.primary }]}>
                    Cancel
                  </Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.confirmModalButton, { backgroundColor: "#FF3B30" }]}
                  onPress={() => {
                    if (selectedActivity) {
                ...
                >
                  <Text
                    style={[
                      ...
                      {
                        color:
                          filterCategory === null ? colors.primary : colors.subtext,
                      },
                    ]}
                  >
                    All Categories
